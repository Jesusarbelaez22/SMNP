...
# Configuración SNMP
SNMP_TARGET = ["127.0.0.1", "192.168.56.101"]  # Lista de direcciones IP
...
def get_snmp_data_with_snmpget(oid):
    """
    Realiza una consulta SNMP para cada dirección IP.
    """
    if not os.path.exists(SNMPGET_PATH):
        print(f"El archivo {SNMPGET_PATH} no se encuentra.")
        return None

    for target in SNMP_TARGET:  # Itera sobre las IPs en SNMP_TARGET
        command = [
            SNMPGET_PATH,
            "-r:" + target,
            "-v:2c",
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if "Value=" in line:
                        ...
                        return value  # Devuelve el primer valor encontrado
            else:
                print(f"Error ejecutando snmpget en {target}: {result.stderr}")

        except Exception as e:
            print(f"Excepción en snmpget para {target}: {e}")

    return None  # Devuelve None si no se obtiene un valor válido
